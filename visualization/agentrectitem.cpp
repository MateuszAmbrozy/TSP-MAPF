...
// void AgentRectItem::animateMoveAgent(QGraphicsScene *scene, const QPointF &newPosition)
// {
//     if (!agentRectItem) return;

//     // Create an animation for the movement of the agentRectItem
//     QPropertyAnimation* animation = new QPropertyAnimation(agentRectItem, "p");
//     animation->setDuration(500);  // Set the duration of the animation in milliseconds (e.g., 500ms for smooth movement)
//     animation->setStartValue(agentRectItem->pos());  // Current position
//     animation->setEndValue(newPosition);  // New position to move to

//     animation->start(QAbstractAnimation::DeleteWhenStopped);  // Start the animation and delete it once it's finished
// }
